...
impl GraphicsPipelineClient {
    ...
    fn decode_avc420(&mut self, surface_id: u16, dest_rect: &ExclusiveRectangle, bitmap_data: &[u8]) -> PduResult<()> {
        ...
        if stream.rectangles.is_empty() {
            return Err(pdu_other_err!("AVC420 stream has no region rectangles"));
        }

        // RFX_AVC420_BITMAP_STREAM carries one macroblock-aligned decoded
        // bounding frame plus a surface-coordinate region mask. The metadata
        // is informational for the encoder parameters, but the region
        // rectangles are the required crop mask. Emit one update per region so
        // pixels outside the mask are never published to the destination
        // surface.
        for region in &stream.rectangles {
            ...
            let destination_bottom = u32::from(dest_rect.bottom);
            ...
            {
                return Err(pdu_other_err!("AVC420 region exceeds destination rectangle"));
            }

            let crop_left = region_left - destination_left;
            let crop_top = region_top - destination_top;
            let width = region_right - region_left;
            let height = region_bottom - region_top;
            let destination_rectangle = ExclusiveRectangle {
                left: u16::try_from(region_left)
                    .map_err(|_| pdu_other_err!("AVC420 region left edge exceeds u16"))?,
                top: u16::try_from(region_top)
                    .map_err(|_| pdu_other_err!("AVC420 region top edge exceeds u16"))?,
                right: u16::try_from(region_right)
                    .map_err(|_| pdu_other_err!("AVC420 region right edge exceeds u16"))?,
                bottom: u16::try_from(region_bottom)
                    .map_err(|_| pdu_other_err!("AVC420 region bottom edge exceeds u16"))?,
            };
            let data = crop_decoded_frame_region(
                frame.data(),
                frame.width(),
                frame.height(),
                crop_left,
            ...
            let update = BitmapUpdate {
                ...
                width: u16::try_from(width)
                    .map_err(|_| pdu_other_err!("AVC420 region width exceeds u16"))?,
                height: u16::try_from(height)
                    .map_err(|_| pdu_other_err!("AVC420 region height exceeds u16"))?,
            };
...
/// Crop an arbitrary region from a decoded RGBA frame.
fn crop_decoded_frame_region(
    data: &[u8],
    decoded_width: u32,
    decoded_height: u32,
    left: u32,
    ...
    let right = left
        .checked_add(width)
        .ok_or_else(|| pdu_other_err!("decoded region right edge overflows"))?;
    let bottom = top
        .checked_add(height)
        .ok_or_else(|| pdu_other_err!("decoded region bottom edge overflows"))?;
    if width == 0
        || height == 0
        || right > decoded_width
        || bottom > decoded_height
    {
        return Err(pdu_other_err!("decoded region exceeds decoded frame"));
    }

    let source_stride = decoded_width
        .checked_mul(4)
        .ok_or_else(|| pdu_other_err!("decoded source stride overflows"))?;
    let target_stride = width
        .checked_mul(4)
        .ok_or_else(|| pdu_other_err!("decoded target stride overflows"))?;
    let target_len = target_stride
        .checked_mul(height)
        .ok_or_else(|| pdu_other_err!("decoded target size overflows"))?;
    let target_len = usize::try_from(target_len)
        .map_err(|_| pdu_other_err!("decoded target size exceeds usize"))?;
    let mut cropped = Vec::with_capacity(target_len);
    let target_stride = usize::try_from(target_stride)
        .map_err(|_| pdu_other_err!("decoded target stride exceeds usize"))?;
    for row in 0..height {
        let source_offset = top
            .checked_add(row)
            .and_then(|value| value.checked_mul(source_stride))
            .and_then(|value| value.checked_add(left.checked_mul(4)?))
            .ok_or_else(|| pdu_other_err!("decoded source offset overflows"))?;
        let source_offset = usize::try_from(source_offset)
            .map_err(|_| pdu_other_err!("decoded source offset exceeds usize"))?;
        let source_end = source_offset
            .checked_add(target_stride)
            .ok_or_else(|| pdu_other_err!("decoded source slice overflows"))?;
        let source = data
            .get(source_offset..source_end)
            .ok_or_else(|| pdu_other_err!("decoded frame data is truncated"))?;
        cropped.extend_from_slice(source);
    }
    Ok(cropped)
}
...
mod tests {
    ...
    struct TestDecoder;
    impl H264Decoder for TestDecoder {
    ...
    #[derive(Clone)]
    struct CaptureHandler(Arc<Mutex<Vec<(ExclusiveRectangle, Vec<u8>)>>>);
    impl GraphicsPipelineHandler for CaptureHandler {
        fn on_bitmap_updated(&mut self, update: &BitmapUpdate) {
            self.0
                .lock()
                .expect("capture handler mutex")
    ...
    #[test]
    fn crop_decoded_frame_region_identity() {
        let data = vec![0xFFu8; 4 * 4 * 4];
        let cropped = crop_decoded_frame_region(&data, 4, 4, 0, 0, 4, 4).unwrap();
        assert_eq!(cropped.len(), data.len());
    }

    #[test]
    fn crop_decoded_frame_region_handles_macroblock_alignment() {
        // H.264 encodes 1920x1080 as 1920x1088 (rounded to 16-pixel macroblock boundary)
        let data = vec![0xAAu8; 1920 * 1088 * 4];
        let cropped = crop_decoded_frame_region(&data, 1920, 1088, 0, 0, 1920, 1080).unwrap();
        assert_eq!(cropped.len(), 1920 * 1080 * 4);
    }

    #[test]
    fn crop_decoded_frame_region_preserves_nonzero_origin() {
        let data = vec![
            0x01, 0, 0, 0xff, 0x02, 0, 0, 0xff, 0x03, 0, 0, 0xff,
            0x04, 0, 0, 0xff, 0x05, 0, 0, 0xff, 0x06, 0, 0, 0xff,
            0x07, 0, 0, 0xff, 0x08, 0, 0, 0xff, 0x09, 0, 0, 0xff,
        ];
        let cropped = crop_decoded_frame_region(&data, 3, 3, 1, 1, 2, 2).unwrap();
        assert_eq!(cropped[0], 0x05);
        assert_eq!(cropped[4], 0x06);
        assert_eq!(cropped[8], 0x08);
        assert_eq!(cropped[12], 0x09);
    }

    #[test]
    fn avc420_region_mask_emits_one_update_per_region() {
        ...
        let regions = [
            Avc420Region::new(11, 21, 12, 22, 22, 100),
            Avc420Region::new(10, 20, 10, 20, 22, 100),
        ];
        let bitmap_data = encode_avc420_bitmap_stream(&regions, &[0, 0, 0, 1, 0x65]);
        ...
        client
            .decode_avc420(7, &destination, &bitmap_data)
            .expect("valid AVC420 region mask");

        let updates = captured.lock().expect("capture handler mutex");
        assert_eq!(updates.len(), 2);
        assert_eq!(updates[0].0.left, 11);
        assert_eq!(updates[0].0.top, 21);
        assert_eq!(updates[0].0.right, 13);
        assert_eq!(updates[0].0.bottom, 23);
        assert_eq!(updates[0].1[0], 5, "crop starts at bounding-frame pixel (1, 1)");
        assert_eq!(updates[1].0.left, 10);
        assert_eq!(updates[1].0.top, 20);
