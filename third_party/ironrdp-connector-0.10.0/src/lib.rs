...
pub struct Config {
    ...
    /// Advertise the RDP Graphics Pipeline dynamic virtual channel.
    ///
    /// The upstream 0.10 connector did not expose the
    /// `SUPPORT_DYN_VC_GFX_PROTOCOL` early capability flag even though it
    /// accepts graphics DVC processors.  Keep this opt-in so legacy clients
    /// retain the upstream wire behavior and only a caller that has installed
    /// a complete EGFX decoder advertises the channel.
    pub support_dynvc_gfx_protocol: bool,
}
