...
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $package), $attempt | Out-Null
try {
    ...
    $manifest.correspondingSource.sha256 = (Get-FileHash -LiteralPath $sourceAssetPath -Algorithm SHA256).Hash
    ...
    foreach ($temporary in @($attempt, $backup)) {
...
Write-Host "Corresponding source sibling asset: $sourceAssetPath"
