...
#[derive(Debug, Parser)]
#[command(
    name = "freeremotedesk-linux",
    about = "FreeRemoteDesk Linux 单窗口客户端"
)]
pub(crate) struct Cli {
    ...
    #[arg(
        long,
        value_enum,
        help = "启用默认关闭的 RDP EGFX 验证实验（需要受信随包 FFmpeg 后端）"
    )]
    pub(crate) rdp_egfx_experiment: Option<RdpEgfxExperiment>,
    #[arg(long, hide = true, conflicts_with_all = ["connect", "rdp_egfx_experiment", "verify_codec_bundle"])]
    test_texture: bool,
    #[arg(long, hide = true, conflicts_with_all = ["connect", "rdp_egfx_experiment"])]
...
mod tests {
    ...
    fn no_arguments_launch_the_single_connection_form() {
        let cli = Cli::try_parse_from(["freeremotedesk-linux"]).unwrap();
    ...
    fn literal_password_option_does_not_exist() {
        let error =
            Cli::try_parse_from(["freeremotedesk-linux", "--password", "must-not-enter-argv"])
    ...
    #[test]
    fn offline_modes_cannot_silently_consume_live_connection_requests() {
        for flags in [
            vec!["--test-texture", "--connect"],
            vec!["--verify-codec-bundle", "--connect"],
            vec!["--test-texture", "--verify-codec-bundle"],
            vec!["--test-texture", "--rdp-egfx-experiment", "avc420"],
            vec!["--verify-codec-bundle", "--rdp-egfx-experiment", "avc444"],
        ] {
            let error = Cli::try_parse_from(std::iter::once("app").chain(flags)).unwrap_err();
