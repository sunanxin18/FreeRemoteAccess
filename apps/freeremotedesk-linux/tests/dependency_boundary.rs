#[test]
fn linux_product_uses_its_platform_adapter_and_shared_protocols() {
    let manifest = include_str!("../Cargo.toml");
    assert!(manifest.contains("frd-platform-linux"));
    ...
    assert!(entry.contains("RdpClientPlatformIdentity::Unix"));
}

#[test]
fn linux_production_composition_keeps_egfx_live_gate_closed() {
    let entry = include_str!("../src/main.rs");
    assert!(
        entry.contains("legacy_rdp_factory(platform)"),
        "Linux composition must keep the legacy fallback when the codec bundle is unavailable"
    );
    assert!(
        entry.contains("fn legacy_rdp_factory(platform: RdpClientPlatformIdentity)"),
        "Linux composition must retain an explicit legacy factory boundary"
    );
    assert!(
        entry.contains("RdpGraphicsAdvertisementGate::ValidationOnly"),
        "Linux opt-in experiment must use validation-only gate"
    );
    assert!(
        !entry.contains("RdpGraphicsAdvertisementGate::LiveInteroperable"),
        "Linux production composition must not carry a live interoperability claim"
    );
}
