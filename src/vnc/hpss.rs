...
use crate::vnc::media_negotiation::{self, MediaStreamAnswer};
use crate::vnc::media_protocol::{self, MediaStreamPortAnnouncement};
use crate::vnc::media_transport::{MediaDatagram, MediaRole, MediaTransport, MediaTransportPhase};
use crate::vnc::mvs::{self, MvsRecordKind, MVS_FULL_FRAME_SIGNATURE};
use crate::vnc::mvs_stream::{MvsRecord, MvsRecordAssembler, MvsRect, MAX_MVS_RECORD_PAYLOAD};
use crate::vnc::protocol;
use crate::vnc::srtp::parse_rtp_header;
...
pub mod cursor {
    ...
    pub const MAX_PAYLOAD_BYTES: usize = 4 * crate::vnc::protocol::limits::BINARY_MEBIBYTE_BYTES;
}
...
const MAX_AUDIO_RTP_CAPTURE_BYTES: usize = 16 * crate::vnc::protocol::limits::BINARY_MEBIBYTE_BYTES;
...
pub(crate) fn mvs_envelope_candidate_rect(message: &[u8]) -> Option<MvsRect> {
...
pub fn run(
    ...
    while Instant::now() < deadline {
        ...
        let m = match conn.read_app_frame() {
            Ok(m) => m,
            Err(e) if crate::vnc::client::is_timeout(&e) => {
                if media_transport.phase() == MediaTransportPhase::Active {
            ...
            Err(e)
                if crate::vnc::client::is_peer_closed(&e)
...
mod tests {
    ...
    use crate::vnc::media_protocol::parse_media_stream_port_announcement;
    use crate::vnc::srtp::{
