...
impl SessionEncodingProfile {
    fn encodings(self) -> Vec<i32> {
        match self {
            ...
            Self::AppleUdpMedia => vec![protocol::MEDIA_STREAM_CONTROL_ENCODING],
        }
    }
}
...
mod tests {
    ...
    const AES_APPLE_UDP_MEDIA_NEGOTIATION_FIXTURE: [u8; 24] = [
        0x12, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x0a, 0x00, 0x00,
        0x01, 0x02, 0x00, 0x00, 0x01, 0x00, 0x00, 0x03, 0xf2,
    ];
    ...
    #[test]
    fn session_encoding_profile_udp_media_is_strictly_media_control_only() {
        assert_eq!(
            SessionEncodingProfile::AppleUdpMedia.encodings(),
            [protocol::MEDIA_STREAM_CONTROL_ENCODING]
        );
    }
    ...
    fn parse_encryption_info_derives_key() {
        ...
        let (counter, crypto) = SessionCrypto::parse_encryption_info(&initial, &msg).unwrap();
        assert_eq!(counter, 1);
        // 解出的钥能直接封帧并被同钥对端解开
        let mut peer = SessionCrypto::from_key_iv(new_key, new_iv);
        let (_, mut outbound) = crypto.split();
