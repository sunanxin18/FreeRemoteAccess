...
mod native {
    ...
    use frd_video_ffmpeg::abi::{
        FrdByteSlice, FrdDecodedFrame, FrdDecodedPlane, FrdDecoderHandle, FrdOwnedBuffer,
        FrdStatus, FrdVideoConfig, RawFrdFfmpegApiV1, FRD_API_CONTRACT_REQUIRED,
        FRD_BITSTREAM_ANNEX_B, FRD_CHROMA_YUV_420, FRD_CHROMA_YUV_444, FRD_CODEC_CAP_H264_AVC420,
        FRD_CODEC_CAP_H264_AVC444, FRD_CODEC_CAP_HEVC_MAIN_444_8, FRD_CODEC_H264, FRD_CODEC_HEVC,
        FRD_FFMPEG_ABI_VERSION, FRD_FFMPEG_API_V1_ALIGNMENT, FRD_FFMPEG_API_V1_SIZE,
    ...
    unsafe extern "C" {
        ...
        fn frd_native_decoder_create_h264_with_thread_policy(
            extradata: *const u8,
            extradata_len: usize,
            width: i32,
            height: i32,
        ...
        fn frd_native_decoder_create_h264_444_with_thread_policy(
            extradata: *const u8,
            extradata_len: usize,
            width: i32,
            height: i32,
    ...
    unsafe extern "C" fn create_decoder(
        ...
        let status = unsafe {
            match config.codec {
                FRD_CODEC_H264 if config.profile == FRD_PROFILE_H264_AVC444 => {
                    frd_native_decoder_create_h264_444_with_thread_policy(
                        extradata.as_ptr(),
                        extradata.len(),
                ...
                FRD_CODEC_H264 => frd_native_decoder_create_h264_with_thread_policy(
                    extradata.as_ptr(),
                    extradata.len(),
                    config.coded_width as i32,
                    config.coded_height as i32,
                ...
                FRD_CODEC_HEVC => frd_native_decoder_create_with_thread_policy(
                    extradata.as_ptr(),
                    extradata.len(),
                    config.coded_width as i32,
                    config.coded_height as i32,
    ...
    unsafe extern "C" fn receive(
        ...
        if let Some(frame) = state.queued.pop_front() {
            ...
            unsafe { publish_frame(frame, state.width, state.height, state.pixel_format, output) };
            return FrdStatus::OK;
        }
        ...
        match receive_native(state) {
            Ok(frame) => {
                ...
                unsafe {
                    publish_frame(frame, state.width, state.height, state.pixel_format, output)
                };
                FrdStatus::OK
            }
            Err(status) => status,
    ...
    fn validate_config(config: &FrdVideoConfig) -> Result<(), FrdStatus> {
        let supported = (config.codec == FRD_CODEC_HEVC
            && config.profile == FRD_PROFILE_HEVC_MAIN_444_8
            && config.chroma == FRD_CHROMA_YUV_444)
            || (config.codec == FRD_CODEC_H264
                && config.profile == FRD_PROFILE_H264_AVC420
                && config.chroma == FRD_CHROMA_YUV_420)
            || (config.codec == FRD_CODEC_H264
                && config.profile == FRD_PROFILE_H264_AVC444
                && config.chroma == FRD_CHROMA_YUV_444);
        if !supported || config.bit_depth != 8 || config.bitstream_format != FRD_BITSTREAM_ANNEX_B {
        ...
        checked_frame_bytes(config.coded_width, config.coded_height, config.chroma)?;
        Ok(())
    }

    fn annex_b_extradata(config: &FrdVideoConfig) -> Result<Vec<u8>, FrdStatus> {
        let sets: [FrdByteSlice; 3] = if config.codec == FRD_CODEC_HEVC {
            [config.vps, config.sps, config.pps]
        } else {
            [
                FrdByteSlice {
                    data: ptr::null(),
    ...
    fn copy_frame(state: &DecoderState, view: NativeFrameView) -> Result<OwnedFrame, FrdStatus> {
        ...
        let expected_format = if state.pixel_format == FRD_CHROMA_YUV_420 {
            unsafe { frd_native_yuv420p_format() }
        } else {
        ...
        let frame_bytes = checked_frame_bytes(state.width, state.height, state.pixel_format)?;
        let mut planes = Vec::with_capacity(3);
        for index in 0..3 {
            let (width, height) =
                plane_extent(state.width, state.height, state.pixel_format, index)?;
            let stride =
    ...
    unsafe fn publish_frame(
        ...
        for (index, (target, allocation)) in planes.iter_mut().zip(frame.planes).enumerate() {
            let len = allocation.len();
            let data = Box::into_raw(allocation).cast::<u8>();
            let (plane_width, plane_height) = plane_extent(width, height, pixel_format, index)
                .expect("validated decoder frame plane extent");
            *target = FrdDecodedPlane {
                width: plane_width as u32,
                height: plane_height as u32,
                stride_bytes: plane_width as u32,
                buffer: FrdOwnedBuffer { data, len },
            };
        }
        let decoded = FrdDecodedFrame {
            timestamp_ticks: frame.timestamp_ticks,
            pixel_format: if pixel_format == FRD_CHROMA_YUV_420 {
                FRD_PIXEL_FORMAT_YUV_420_P8
            } else {
    ...
    fn checked_frame_bytes(width: u32, height: u32, pixel_format: u32) -> Result<usize, FrdStatus> {
    ...
    fn plane_extent(
        ...
        pixel_format: u32,
        index: usize,
    ) -> Result<(usize, usize), FrdStatus> {
        if index >= 3 || width == 0 || height == 0 {
            return Err(FrdStatus::INVALID_ARGUMENT);
        }
        let width = usize::try_from(width).map_err(|_| FrdStatus::INVALID_ARGUMENT)?;
        let height = usize::try_from(height).map_err(|_| FrdStatus::INVALID_ARGUMENT)?;
        if pixel_format == FRD_CHROMA_YUV_420 && index > 0 {
            Ok((width.div_ceil(2), height.div_ceil(2)))
        } else {
            Ok((width, height))
        }
    }
    ...
    mod tests {
        ...
        fn test_state(width: u32, height: u32) -> DecoderState {
            DecoderState {
                ...
                width,
                height,
                pixel_format: FRD_CHROMA_YUV_444,
                queued: VecDeque::new(),
                queued_bytes: 0,
            }
        }
