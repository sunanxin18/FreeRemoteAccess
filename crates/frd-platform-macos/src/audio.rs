...
#[cfg(target_os = "macos")]
impl AudioOutput for MacAudioOutput {
    fn enqueue_pcm(
        &mut self,
        sample_rate_hz: u32,
        channels: u8,
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    #[ignore = "仅显式运行：打开本机默认音频设备并发送静音"]
    fn native_coreaudio_opens_and_accepts_stereo_silence() {
        use frd_media_api::AudioOutput;
        let mut output = super::MacAudioOutput::open_default().expect("CoreAudio 默认设备");
        output
            .enqueue_pcm(48_000, 2, vec![0; 960].into_boxed_slice())
            .unwrap();
