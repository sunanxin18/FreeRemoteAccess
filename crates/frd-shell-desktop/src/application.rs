#[cfg(test)]
use crate::session_host::TestLaunchOutcome;
...
use frd_core::{
    ...
    PixelSize, PointerButton, ProtocolId, ResolutionMode, SessionId,
};
use frd_frame::{
    FrameCompleteness, FrameReset, FrameRevision, FrameTransaction, FrameTransactionError,
    PixelBuffer, PixelFormat, PixelPatch,
};
use frd_media_api::{
    AudioOutput, AudioOutputError, DecodedVideoFrame, MediaStageDiagnostic, MediaStageTrace,
    VideoDecodeErrorCode, VideoStreamIdentity,
};
use frd_protocol_api::{
    ProtocolCatalog, ProtocolError, ProtocolFactory, SessionCommand, SessionEvent,
};
use frd_render_wgpu::{
...
use crate::cleanup::BackgroundCleanupOutcome;
use crate::fatal::{FatalComponent, FatalOperation, FatalReason, FatalReport};
use crate::frame_metrics::{FramePipelineMetrics, MetricIdentity};
#[cfg(test)]
...
use crate::video_decode_worker::{VideoFrameToken, VideoStreamAdmission, VideoWorkerEvent};
...
mod tests {
    ...
    use super::{
        ...
        FrameDrainFailure, RemoteBinding, RuntimeDrainCallback, RuntimeDrainCallbackTarget,
        RuntimeDrainOutcome, RuntimeWakeGate, SessionHost, TestLaunchOutcome,
        UnavailableCredentialStore, UnavailableProfileStore, VideoBinding, VideoSurfaceDrainTarget,
        WakeSink,
    };
    use crate::frame_metrics_sink::MetricSinkError;
    use crate::session_host::test_support::{WorkerKind, WorkerSpawner};
    ...
    fn frame_drain_host(session_id: SessionId) -> SessionHost {
        ...
        host.attach_frame_test_mailbox(session_id, mailbox);
        host
    }
    ...
    fn protocol_spawn_failure_returns_from_dispatch_before_off_loop_rollback_reclaims_audio() {
        ...
        let mut host = SessionHost::new_with_test_spawner(
            [Arc::new(BlockingFactory {
    ...
    fn audio_worker_ignores_video_until_first_pcm_frame() {
        ...
        let video_only_worker = std::thread::spawn(move || {
            crate::session_host::test_support::drain_audio_media(video_only_factory, video_only_rx)
        });
        video_only_tx
        ...
        assert_eq!(
            ...
            crate::session_host::test_support::AudioWorkerExit::Closed
        );
        assert_eq!(
        ...
        let video_then_pcm_worker = std::thread::spawn(move || {
            crate::session_host::test_support::drain_audio_media(
                video_then_pcm_factory,
                video_then_pcm_rx,
            )
        });
        video_then_pcm_tx
    ...
    fn audio_drain_remains_independent_while_video_backend_load_is_blocked() {
        ...
        let publisher =
            crate::session_host::test_support::media_publisher(audio_tx, video_worker.sender());
        let pcm_frames = Arc::new(Mutex::new(Vec::new()));
        ...
        let audio_worker = std::thread::spawn(move || {
            crate::session_host::test_support::drain_audio_media(audio_factory, audio_rx)
        });
        let identity = VideoStreamIdentity {
            session_id: SessionId::allocate(),
            stream_id: 9,
    ...
    fn assert_protocol_worker_waits_until_live_ports_are_installed(
        ...
        assert!(host.active_session_id().is_some());
    ...
    fn assert_cancelled_started_probe_launch(target: TargetSystem, protocol_id: ProtocolId) {
        ...
        assert!(
            host.active_session_id().is_none(),
