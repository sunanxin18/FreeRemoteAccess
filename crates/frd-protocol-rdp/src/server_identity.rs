...
fn classify_identity(
    ...
    match saved_pin {
        ...
        Some(_) => IdentityDisposition::PinMismatch,
...
mod tests {
    ...
    fn server_identity_unknown_self_signed_certificate_requires_a_challenge() {
        assert_eq!(
            classify_identity(None, FINGERPRINT, Err(PlatformValidationFailure), names(),),
            IdentityDisposition::Challenge {
                fingerprint: FINGERPRINT,
    ...
    fn server_identity_tls_preflight_writes_no_application_data_and_exact_pin_reconnects() {
        let (address, server, certificate_der) = spawn_tls_server(2);
