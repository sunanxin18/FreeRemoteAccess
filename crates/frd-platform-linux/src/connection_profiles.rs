...
pub struct LinuxConnectionProfileStore {
    path: PathBuf,
}

impl LinuxConnectionProfileStore {
    ...
    fn lock(&self) -> Result<super::single_instance::LinuxSingleInstanceGuard, PlatformError> {
        ...
        lock.push(".lock");
        super::single_instance::LinuxSingleInstanceGuard::lock_at(&PathBuf::from(lock), false)
            .map_err(|_| PlatformError::StorageFailed)
    }

    fn read_profiles(&self) -> Result<Vec<SavedConnectionProfile>, PlatformError> {
        let bytes = match super::paths::read(&self.path)? {
            Some(bytes) => bytes,
    ...
    fn write_profiles(&self, profiles: &[SavedConnectionProfile]) -> Result<(), PlatformError> {
        ...
        super::paths::write_atomic(&self.path, &encoded)
    }
}

impl ConnectionProfileStore for LinuxConnectionProfileStore {
...
mod tests {
    ...
    use super::LinuxConnectionProfileStore;
    ...
    fn metadata_round_trip_uses_only_the_version_one_schema_keys() {
        ...
        let store = LinuxConnectionProfileStore::at_path(path.clone());
    ...
    fn metadata_lists_newest_success_first() {
        let temporary = crate::private_tempdir();
        let store = LinuxConnectionProfileStore::at_path(
            temporary
                .path()
                .canonicalize()
                .unwrap()
    ...
    fn metadata_rejects_malformed_records() {
        ...
        let store = LinuxConnectionProfileStore::at_path(path);
    ...
    fn concurrent_store_instances_preserve_each_profiles_update() {
        ...
        let path = dir.path().canonicalize().unwrap().join("connections.json");
        std::thread::scope(|scope| {
            for index in 0..12 {
                let path = &path;
                scope.spawn(move || {
                    LinuxConnectionProfileStore::at_path(path)
                        .upsert(&test_profile(
