...
| Linux | Vulkan/GLES 条件后端及独立 desktop GL 执行器（x86_64/ARM64 软件 EGL fixture 已通过，i686 待修正 CI 重跑）；GTK/GLArea 独立探针三架构 X11 1× 绘制及输入报告通过离线复验，产品 shell 开发中 | 计划中 | i686/x86_64/ARM64 完整包及目标进程解码器加载同轮通过（`09565d3` run `34206959387`） | 尚无已验证的产品组合 | **开发中**；2026-09-08 独立平台服务、应用入口、Ubuntu x86_64 临时 Secret Service 往返及原生应用测试已通过。09565d3 原生探针三架构 X11 1×/2× 输入及几何通过，x86_64/i686 Wayland 1× 通过；Wayland 2× 实际缩放和 ARM64 日志串流问题导致本轮失败，待修正重跑。软件 Mesa 探针不是硬件 GPU 或产品 RDP 控制证明；完整 GTK 产品接线和真实控制仍未完成。见 [基础服务验证](docs/validation/linux-client-foundation-20260908.md)和 [Linux 计划](docs/superpowers/plans/2026-09-08-linux-client.md)。 |
