...
| Linux | Vulkan/GLES 条件后端及独立 desktop GL 执行器（三架构软件 EGL fixture 已通过）；GTK/GLArea 三架构 X11/Wayland 1×/2× 原生 frame、窗口提交和 runner fixture 已通过 | 计划中 | i686/x86_64/ARM64 完整包及目标进程解码器加载同轮通过；stage 现在包含固定 Noto Sans SC 中文字体并由 verifier 做大小/hash/权限检查 | 尚无已验证的产品组合 | **开发中**；2026-09-08 GTK observer gap 修正后的 `34223764518` 三架构 X11/Wayland 1×/2× 均通过完整帧、增量、上下文重建、登录/取消、窗口提交、故障拒绝和 next-UPDATE 消费。`GtkRunner` 已接入当前 GDK 设备的 XKB→USB HID 键盘、同一物理 `ContentViewport` 的鼠标/滚轮，以及代际/失焦/移出时的旧代际 `ReleaseAll`；Linux 包加入窗口树私有 PangoCairo 字体回退。该轮 CI 证据早于输入/字体接线，新的 Linux 编译和真实 X11/Wayland 输入仍待下一轮 CI；Linux 产品入口和真实 RDP 控制仍未完成。软件 Mesa 不是硬件 GPU 或产品互操作证明。见 [基础服务验证](docs/validation/linux-client-foundation-20260908.md)和 [Linux 计划](docs/superpowers/plans/2026-09-08-linux-client.md)。 |
