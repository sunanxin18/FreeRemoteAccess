...
| macOS / Linux native video backend | **开发中** | macOS FFmpeg builder/stager/verifier 现在接受显式 x86_64/arm64 profile，并按目标 Rust target、Clang `-arch`、Mach-O 架构和 NASM/NEON 门禁生成独立 bundle；2026-09-08 本机完成 arm64 与 x86_64 FFmpeg bundle、Rust plugin、`.app` staging/verifier，两个架构各完成 6 项 native fixture，x86_64 在 Rosetta 下执行。新增 `.github/workflows/build-macos.yml` 为 macOS-15 arm64 与 macOS-15-intel x86_64 建立独立的 Rust target、原生 fixture、应用 staging/verifier 和 artifact gate，staging 使用与构建相同的架构专属 `FRD_FFMPEG_BUILD_ROOT`、`Cargo.lock`，并校验 runner 原生架构；workflow 另上传按架构分开的 FFmpeg 8.1.2 对应源码/hash evidence artifact。Linux 协议中立 core crates、`frd-video-ffmpeg` loader 和不启用 native C ABI 的 plugin crate 已在固定 Rust 1.96.0 下通过 `x86_64-unknown-linux-gnu`、`i686-unknown-linux-gnu`、`aarch64-unknown-linux-gnu` 三个 target check。Linux FFmpeg 仍有固定架构目录、同一 owner、非 group/other writable、无 symlink 路径门禁，并按依赖顺序以 `RTLD_GLOBAL` 加载 FFmpeg 依赖；新增 `tools/verify-linux-ffmpeg-bundle.sh`，对 Linux x86_64/i686/arm64 检查精确文件集合、ELF 架构、同一 owner UID、非 group/other writable 文件、无 symlink 路径组件、SONAME/随包依赖、插件 ABI 导出；构建脚本按实际 `CARGO_TARGET_DIR` 定位 plugin，避免隔离构建误取旧产物；x86_64 hosted profile 还会调用 ABI v1 入口，交叉架构保持静态 gate。`build-linux-ffmpeg.yml` 现在为 x86_64、i686 和 arm64 建立独立的 Rust target、FFmpeg cross compiler、NASM/NEON、ABI plugin、bundle 以及对应源码/hash evidence artifact gate；对应托管 job 尚未在本机执行，Linux native bundle、GUI 包与真机运行验证仍未完成。 |
