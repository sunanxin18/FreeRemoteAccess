...
| Linux | Vulkan/GLES 条件后端及独立 desktop GL 执行器（待 Linux 原生验证）；GTK/GLArea 独立探针三架构 X11 1× 绘制及输入报告通过离线复验，产品 shell 开发中 | 计划中 | i686/x86_64/ARM64 完整包及目标进程解码器加载同轮通过（`a60eebd` run `34202134581`） | 尚无已验证的产品组合 | **开发中**；2026-09-08 独立平台服务、应用入口、Ubuntu x86_64 临时 Secret Service 往返及原生应用测试已通过。原生探针首轮 CI 因标题栏边框坐标校验失败；修正后对三架构 X11 1× 实际报告复验通过，2× 与 Wayland 待复跑。软件 Mesa 探针不是硬件 GPU 或产品 RDP 控制证明；完整 GTK 产品接线和真实控制仍未完成。见 [基础服务验证](docs/validation/linux-client-foundation-20260908.md)和 [Linux 计划](docs/superpowers/plans/2026-09-08-linux-client.md)。 |
