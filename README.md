...
2026-09-09 当前进度：macOS 仅支持 ARM64。加入 Linux GL 后端后的 macOS ARM64 完整工作区测试 1720 passed / 0 failed / 16 ignored；Linux 三架构最新完整包 run `34263312162`、Windows 三架构包及 ARM64 原生 decoder runtime run `34255887161`、macOS ARM64 最新完整包 run `34263312174` 均成功；默认生产 RDP 仍为 LegacyOnly，显式 `--rdp-egfx-experiment avc420|avc444` 用于采集互操作证据。
...
|---|---|---|---|---|---|
| Windows | winit + egui + wgpu | 键盘、鼠标 | x86_64 默认 package；x86/i686 与 arm64/aarch64 使用同一显式架构 profile 生成独立 codec 目录和 manifest；MSI/MSIX 仍开发中 | macOS；Windows RDP 开发中 | **开发中**；固定 Rust 1.96.0 的协议中立 core/video/plugin crates 已通过 MSVC x86_64/i686/arm64 target check；统一视频 decoder 的编译、离线 fixture、DX12 readback、package staging 与 codec present/absent 单实例 GUI 门禁已完成；`.github/workflows/build-windows.yml` 已扩展 Rust MSVC x86_64/i686/arm64、WSL MinGW、NASM/x86asm 与 AArch64/NEON 的分架构 package/verifier gate，verifier 现在还按 `0x8664`/`0x014c`/`0xAA64` 精确检查 EXE 与 codec DLL 的 PE Machine，`34255887161` 的三架构包及 Windows ARM64 原生 ClearCodec/NSCodec/Progressive runtime 已通过；该证据不替代真实 GUI 控制。Apple Standard/HP 与 RDP 的当前真机边界见 [`cross-platform-video-decoder-20260901.md`](docs/validation/cross-platform-video-decoder-20260901.md)；RDP 仍等待独立授权的原生 Windows 目标完成登录、首帧与输入门禁。 |
