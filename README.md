...
2026-09-09 当前进度：macOS 仅支持 ARM64。加入 Linux GL 后端后的 macOS ARM64 完整工作区测试 1720 passed / 0 failed / 16 ignored；Linux 三架构最新完整包 run `34263312162`、Windows 三架构最新包 run `34262976196` 及 ARM64 原生 decoder runtime job `102205434464`、macOS ARM64 最新完整包 run `34263312174` 均成功；默认生产 RDP 仍为 LegacyOnly，显式 `--rdp-egfx-experiment avc420|avc444` 用于采集互操作证据。
...
|---|---|---|---|---|---|
| Windows | winit + egui + wgpu | 键盘、鼠标 | x86_64 默认 package；x86/i686 与 arm64/aarch64 使用同一显式架构 profile 生成独立 codec 目录和 manifest；MSI/MSIX 仍开发中 | macOS；Windows RDP 开发中 | **开发中**；固定 Rust 1.96.0 的协议中立 core/video/plugin crates 已通过 MSVC x86_64/i686/arm64 target check；统一视频 decoder 的编译、离线 fixture、DX12 readback、package staging 与 codec present/absent 单实例 GUI 门禁已完成；`.github/workflows/build-windows.yml` 已扩展 Rust MSVC x86_64/i686/arm64、WSL MinGW、NASM/x86asm 与 AArch64/NEON 的分架构 package/verifier gate，verifier 现在还按 `0x8664`/`0x014c`/`0xAA64` 精确检查 EXE 与 codec DLL 的 PE Machine，`34262976196` 的三架构包及 ARM64 原生 ClearCodec/NSCodec/Progressive runtime job `102205434464` 已通过；该证据不替代真实 GUI 控制。Apple Standard/HP 与 RDP 的当前真机边界见 [`cross-platform-video-decoder-20260901.md`](docs/validation/cross-platform-video-decoder-20260901.md)；RDP 仍等待独立授权的原生 Windows 目标完成登录、首帧与输入门禁。 |
...
| Linux | Vulkan/GLES 条件后端及独立 desktop GL 执行器（三架构软件 EGL fixture 已通过）；GTK/GLArea 三架构 X11/Wayland 1×/2× 原生 frame、窗口提交和 runner fixture 已通过 | 开发中 | i686/x86_64/ARM64 完整包及目标进程解码器加载同轮通过；stage 现在包含固定 Noto Sans SC 中文字体并由 verifier 做大小/hash/权限检查 | 尚无已验证的产品组合 | **开发中**；`34233406400` 在 x86_64、i686、AArch64 上完成 X11/Wayland 1×/2× 的完整帧、增量、上下文重建、登录/取消、窗口提交、故障拒绝和 next-UPDATE 消费；`34244032943` 又在 i686、x86_64、AArch64 上通过正式 GTK Application/ApplicationWindow 入口、X11 窗口身份/焦点/XTEST 键鼠/干净关闭，以及 X11/Wayland 1×/2× GLArea fixture。`34244032991` 三架构软件 GL renderer 也通过；工作区 wgpu 现编译 Vulkan/GLES/Metal，运行时仍由平台组合根选择 Linux Vulkan|GL、Windows DX12、macOS ARM64 Metal。`34263312162` 三架构完整 Linux 客户端包、ELF/依赖校验、目标进程解码器加载和产物上传也通过。原生 runner 日志确认 `font_map_loaded=true`，离屏登录/连接截图中的中文已正常渲染。`GtkRunner` 已接入当前 GDK 设备的 XKB→USB HID 键盘、同一物理 `ContentViewport` 的鼠标/滚轮，以及代际/失焦/移出时的旧代际 `ReleaseAll`；Linux 包加入窗口树私有 PangoCairo 字体回退。Wayland 全局物理输入、硬件 GPU、窗口装饰的真实桌面验收和真实 RDP 控制仍未完成。软件 Mesa、XTEST 和合成事件不构成这些证据。见 [基础服务验证](docs/validation/linux-client-foundation-20260908.md)和 [Linux 计划](docs/superpowers/plans/2026-09-08-linux-client.md)。 |
...
| H.264 AVC420/AVC444 FFmpeg software bridge | **开发中** | 2026-09-09 已在授权 Windows 目标完成 LegacyOnly、显式 AVC420 和显式 AVC444 三次对照 probe：LegacyOnly 1280×720 首帧 1069 ms、20 秒 162 帧并正常回收；AVC420 确认 EGFX `0x80105/0x2` 但实际 `avc420=0`（ClearCodec 84、Progressive 3），AVC444 确认 `0xa0701/0x52` 且服务器报告 AVC 能力但实际 `avc444=0`，两者均按 `probe_requested_avc_not_decoded` 拒绝。固定 FFmpeg 的 Windows/Linux i686、x86_64、ARM64 与 macOS ARM64 七目标 package/runtime fixture 门禁已有证据；macOS Intel 不支持。能力确认和混合编码首帧不代表 H.264 互操作完成；生产仍使用 LegacyOnly，只有显式有界 probe 或 macOS ARM64 验证入口启用 EGFX。ClearCodec 解码失败现在保留非敏感分类，未放宽序号/缓存一致性。详见 [验证记录](docs/validation/rdp-egfx-h264-20260907.md)。 |
