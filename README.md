...
> 2026-09-08 最新真实探针（`d597212`）：macOS ARM64 首次成功解码 Progressive 2次、ClearCodec 4次，runtime接收3帧（首帧1079ms），随后因 `tile flags` 停止。AVC实际解码仍为零，持续刷新和GUI未验收。下方早期记录不代表当前结果；见 [详细证据](docs/validation/rdp-egfx-h264-20260907.md)。
...
| RemoteFX Progressive / EGFX `0x0009` | **开发中** | 2026-09-08 严格 wire/entropy、surface reference 与 codec DAS 生命周期、SSE2/NEON 数值内核及 native tile 桥已实现，本机 ARM64 41 项组合测试通过；独立 FreeRDP C 数值对照通过。已接入 EGFX frame callbacks 与离屏发布，RDP 全量285项测试通过，含1080p的510tile合并与真实PDU升级路径；独立 C RLGR1 合成 fixture 已逐字节复现，entropy 专项9项通过；严格末符号修复后，真实会话首次成功 Progressive 2次、runtime接收3帧，随后因 tile flags 停止；早先的 Publisher 错误亦需持续观察。七目标 runtime 与真实持续刷新尚未验收。见 [`Progressive 实现边界`](crates/frd-protocol-rdp/src/progressive/README.md)。 |
