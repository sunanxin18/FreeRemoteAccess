...
| Linux | Vulkan/GLES 条件后端及独立 desktop GL 执行器（三架构软件 EGL fixture 已通过，347a79e run34209182139）；GTK/GLArea 独立探针三架构 X11 1× 绘制及输入报告通过离线复验，产品 shell 开发中 | 计划中 | i686/x86_64/ARM64 完整包及目标进程解码器加载同轮通过（`09565d3` run `34206959387`） | 尚无已验证的产品组合 | **开发中**；2026-09-08 独立平台服务、应用入口、Ubuntu x86_64 临时 Secret Service 往返及原生应用测试已通过；凭据后台IO与pending内存锁隔离修正通过21项合成回归。347a79e 原生探针三架构 X11 1×/2× 输入及几何、Wayland 1×/2× GL/几何全部通过；实测目标均 RGBA8/LINEAR，显式RGBA8颜色输出已在c349a72三架构软件EGL通过，Rust GTK 帧事务适配层已完成三架构 X11/Wayland 1×/2× 原生 fixture（96cfeba run34212569009），覆盖完整帧/增量/上下文重建；GTK登录/会话runner和EGL窗口提交observer已实现，三架构X11/Wayland 1×/2×登录/取消mock验收通过，窗口提交及GL/EGL故障拒绝、resize/unrealize失效和消费一次已在三架构X11/Wayland 1×/2×通过（f8d3ca8 run34218500195）；自定义分辨率实际请求与返回保留也已通过该原生fixture，runner已接入消费式FramePresented，c18b1f8原生12组submission已通过次UPDATE消费与错误拒绝，runner12组首次绘制Association后退出，已定位合法初始布局观察缺口；首批明暗PNG暴露GTK中文字体回退缺失，输入法归属仍待接线。软件 Mesa 探针不是硬件 GPU 或产品 RDP 控制证明；完整 GTK 登录、输入、会话接线和真实控制仍未完成。见 [基础服务验证](docs/validation/linux-client-foundation-20260908.md)和 [Linux 计划](docs/superpowers/plans/2026-09-08-linux-client.md)。 |
