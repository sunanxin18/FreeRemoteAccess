...
| Linux | Vulkan/GLES 条件后端及独立 desktop GL 执行器（三架构软件 EGL fixture 已通过）；GTK/GLArea 三架构 X11/Wayland 1×/2× 原生 frame、窗口提交和 runner fixture 已通过 | 开发中 | i686/x86_64/ARM64 完整包及目标进程解码器加载同轮通过；stage 现在包含固定 Noto Sans SC 中文字体并由 verifier 做大小/hash/权限检查 | 尚无已验证的产品组合 | **开发中**；`34233406400` 在 x86_64、i686、AArch64 上完成 X11/Wayland 1×/2× 的完整帧、增量、上下文重建、登录/取消、窗口提交、故障拒绝和 next-UPDATE 消费；`34233406570` 的 Linux 三架构完整包与目标解码器加载也通过。原生 runner 日志确认 `font_map_loaded=true`，离屏登录/连接截图中的中文已正常渲染。`GtkRunner` 已接入当前 GDK 设备的 XKB→USB HID 键盘、同一物理 `ContentViewport` 的鼠标/滚轮，以及代际/失焦/移出时的旧代际 `ReleaseAll`；Linux 包加入窗口树私有 PangoCairo 字体回退。当前正式入口已切换 GTK Application/ApplicationWindow，固定 app-id `com.sunanxin18.freeremotedesk`，并新增隔离 X11 产品 smoke（窗口身份、焦点、XTEST 键鼠、干净关闭）与 Wayland app-id/close-request smoke；两者的本轮三架构执行记录尚未取得，Wayland 物理输入和真实 RDP 控制仍未验收。软件 Mesa 不是硬件 GPU 或产品互操作证明。见 [基础服务验证](docs/validation/linux-client-foundation-20260908.md)和 [Linux 计划](docs/superpowers/plans/2026-09-08-linux-client.md)。 |
