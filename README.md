...
| Linux | Vulkan/GLES 条件后端；完整平台 shell 开发中 | 计划中 | 完整客户端 stage/verifier 与三架构 CI 已实现，尚待运行；已有三架构 FFmpeg bundle | 尚无已验证的产品组合 | **开发中**；2026-09-08 独立平台服务已有实现、17 项合成测试及三架构 check；应用入口已实现，宿主编译及 20 项单元/2 项边界测试通过；真实 Secret Service、Linux 原生应用运行与完整包仍未验收，见 [基础服务验证](docs/validation/linux-client-foundation-20260908.md)和 [Linux 计划](docs/superpowers/plans/2026-09-08-linux-client.md)。现有 CI 的解码/编译证据不证明窗口管理器、Secret Service、完整安装包或真实控制；这些门禁独立未完成。 |
