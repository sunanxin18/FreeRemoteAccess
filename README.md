...
| macOS / Linux native video backend | **开发中** | macOS bundle 已有签名/路径门禁；Linux 已加入固定架构目录、同一 owner、非 group/other writable、无 symlink 路径门禁，并按依赖顺序以 `RTLD_GLOBAL` 加载 FFmpeg 依赖。`build-linux-ffmpeg.yml` 现在为 x86_64、i686 和 arm64 建立独立的 Rust target、FFmpeg cross compiler、NASM/NEON、ABI plugin 和 bundle artifact gate；对应托管 job 尚未在本机执行，Linux GUI 包与真机运行验证仍未完成。 |
