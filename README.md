...
| macOS / Linux native video backend | **开发中** | macOS FFmpeg builder/stager/verifier 现在接受显式 x86_64/arm64 profile，并按目标 Rust target、Clang `-arch`、Mach-O 架构和 NASM/NEON 门禁生成独立 bundle；Linux 已加入固定架构目录、同一 owner、非 group/other writable、无 symlink 路径门禁，并按依赖顺序以 `RTLD_GLOBAL` 加载 FFmpeg 依赖。Linux x86_64 native host 可在构建阶段用同一 FFmpeg dist 跑 plugin 单测；新增 `tools/verify-linux-ffmpeg-bundle.sh`，对 Linux x86_64/i686/arm64 检查精确文件集合、ELF 架构、SONAME/随包依赖、插件 ABI 导出；x86_64 hosted profile 还会调用 ABI v1 入口，交叉架构保持静态 gate。`build-linux-ffmpeg.yml` 现在为 x86_64、i686 和 arm64 建立独立的 Rust target、FFmpeg cross compiler、NASM/NEON、ABI plugin 和 bundle artifact gate；对应托管 job 尚未在本机执行，Linux GUI 包与真机运行验证仍未完成。 |
