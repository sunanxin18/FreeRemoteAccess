...
pub struct Config {
    ...
    /// Optional pre-resolved direct TCP address. The destination name remains
    /// authoritative for TLS and RDP server identity.
    pub(crate) connect_addr: Option<core::net::SocketAddr>,
...
impl Config {
    ...
    /// Pre-resolved direct TCP address, when supplied by a bounded detector.
    pub fn connect_addr(&self) -> Option<core::net::SocketAddr> {
        self.connect_addr
    }
...
impl fmt::Debug for Config {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        s.field("connect_addr", &self.connect_addr);
...
pub struct ConfigBuilder {
    ...
    connect_addr: Option<core::net::SocketAddr>,
...
impl ConfigBuilder {
    ...
    /// Pin the direct TCP socket while preserving [`Destination`] as the TLS
    /// and RDP identity. Non-direct transports ignore this value.
    #[must_use]
    pub fn with_connect_addr(mut self, connect_addr: core::net::SocketAddr) -> Self {
        self.connect_addr = Some(connect_addr);
        self
    }
