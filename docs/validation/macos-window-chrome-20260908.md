...
后续本机窗口已可由 CUA 正常读取和操作。使用现有 ARM64 release 应用，
从最近连接加载已保存的 Windows 配置和安全凭据后成功登录，未重新输入密码。
控制岛隐藏时，实际点击 Windows 开始菜单，输入 `notepad` 并提交远程输入法
组合文字，搜索结果随之更新。随后 Escape 返回，Ctrl+Alt+Home 唤出控制岛，
点击远程内容恢复归属并再次隐藏，重复搜索仍成功。最后退出搜索，保留连接。

本轮没有替换应用包、修改远程文件或执行远程终端命令，也没有启用新的实验
图形模式。该证据覆盖当前应用、窗口与连接的两轮原生点击和键盘交互；没有
复现用户报告的失效，不能据此关闭实验 EGFX 的 Progressive coverage 故障。
独立只读审查同样未发现隐藏动作清除焦点或关闭输入 gate 的确定性缺陷。
