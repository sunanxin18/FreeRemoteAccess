...
- A new current final Release identity is intentionally absent from this documentation commit. The primary integrator must append it only after independent build/hash verification; until then the `43db868` artifact above remains merely the prior offline closure build.
