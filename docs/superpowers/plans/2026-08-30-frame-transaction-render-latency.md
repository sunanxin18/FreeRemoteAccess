...
- A GPU fault may occur after one or more `queue.write_texture` calls. After a scope begins, execution never uses `?` or returns before exactly one `finish`; an observed GPU fault wins over a simultaneous execution error. If a scoped operation panics, catch it only long enough to finish and poll exactly once, then resume the original panic; a simultaneous finish panic must not replace it. The batch boundary continues to report an execution panic as `BatchExecutionPanicked`. CPU staged state must not commit, and the shell must block input, detach both `RemoteRenderer` and `RemoteBinding`, clear pending-write bookkeeping, publish stable typed details, and perform no record/submit/present of that texture.
...
- The actual frame port signs an opaque generation admission from its own budget; the desktop binding uses 64 MiB. Apple startup, initial confirmation, dynamic viewport request, exact acknowledgement, and server-geometry paths must obtain that admission before CPU surface allocation/replacement, resized/full wire writes, private-state mutation, generation events, `Reset`, or wake. Zero, arithmetic overflow, or an over-capacity complete surface returns terminal `SurfaceCapacityExceeded`: startup leaves no partial state, while a rejected dynamic transition atomically retains the previous surface and control state.
...
Before statistics, cap each event CSV at 32,768 rows and each process CSV at 62 rows. A valid process CSV has exactly those 62 rows: S0..S30 for each of the two measured phases. Every event row must remain inside the half-open interval for its declared phase, from that phase's boundary through but excluding the next boundary; Restore has no later boundary. Presentation identity is all-or-none and complete values are nonzero. Restore must resolve to one current session and the current generation, with revisions remaining monotonic. Event row 32,769, process row 63, a missing/duplicate S0..S30 sample, an event outside its declared phase, or an invalid Restore identity fails closed and produces no comparison output.
...
Its output includes the exact Task 7 predicates: every `CandidateBatch` row in a performance run has `batch_result=Success` and scope `{1,1,1}`, aggregate candidate begins=finishes=polls=successful non-empty batches, `visible_batch_cpu_8ms_and_no_regression`, `visible_scope_amplification_reduced_50_percent`, the unchanged visible CPU/working-set/input/frame-response gates, minimized CPU/working-set gates, `minimized_presentation_paused`, and complete nonzero current-session/current-generation Restore Presentations from serial and candidate. It exits nonzero for a non-success performance batch, false mandatory predicate, incomplete applicable visible window, missing S0..S30 sample, schema/capacity error, an event outside its declared phase interval, an incomplete/zero/mismatched Restore identity, or non-monotonic identity/timestamp.
...
7. deterministic production-helper lifecycle seams prove that batch execution plus record/presentation error and panic/unwind paths finish/poll every started scope exactly once with GPU fault precedence and clean-token-gated commit/receipt publication; the original operation panic resumes after closure, the batch boundary retains `BatchExecutionPanicked`, and the separate real DX12 smoke (unless adapter unavailable) proves only one clean real-backend scope observes `{1,1,1}`;
...
- Consume only the retained valid fixed-schema captures `serial_capacity_click_20260831_23` and `candidate_capacity_click_20260831_23`. Each stays below the 32,768-row event limit, has exactly 62 process rows containing measured-phase S0..S30, keeps every event inside its declared phase interval, has all five phases, contains zero `StableFault` rows, and supplies complete nonzero current-session/current-generation Restore identity.
...
Run the comparator against the two retained CSV pairs with a new `target/validation` output path. Inspect that JSON to confirm all named mandatory predicates are true, visible metrics retain their worst complete windows, minimized latency/window fields are `null`, minimized Presentation counts are zero, and both Restore receipts carry complete nonzero identity from the current session and generation. Do not alter retained CSV files, connect to the Mac, or infer live interoperability beyond those bounded captures.
...
- Runtime correctness commits `a1099a641f7b1213a88b0059f625941c8a8ef779`, `fed72c61d9d335472a8ca0de9e5c0bda6f22d625`, `0e775379cdc065064f0957b44c73955079c17a8d`, and `91d7c2c422b249870395fe7693133d957d4a4ca2` are post-measurement. Follow-up `43fb0f3fcc06f53a74e1de0671909b58cecc9653` makes the actual-port 64 MiB capacity check an opaque admission for every Apple production generation entry before CPU allocation/replacement, resized/full wire writes, private-state mutation, generation event, `Reset`, or wake; rejection is terminal, leaves no partial startup, and atomically retains the previous dynamic surface/control state. This has offline source/test/workspace evidence only.
- Follow-up `3a7bd2be41f5b10f79ef1e97362f1fab3373419b` makes each successfully started production scope finish/poll exactly once before the original operation panic resumes, without replacing the batch wrapper's `BatchExecutionPanicked` contract. Deterministic production-helper tests cover record/presentation error, fault-precedence, commit/receipt, and panic/unwind paths; the separate Windows DX12 smoke observes only a clean real-backend scope `{1,1,1}`. Neither evidence set is a new Mac run.
- Comparator-only commits `323d6e0aea1cb0bd075f90dcc1973edbc9278f66`, `d36e045541587e56f3af1dee0c0d4477d0686793`, `4344183dadf0e2fb205baa61e38d75395c95ea97`, and `43db868a637ed1fdf13fa8301f900e513b540afa` enforce the 32,768-event/62-process input limits, strict event phase intervals, and complete nonzero current-session/current-generation Restore identity. `target/validation/task7-scheme-a-final-integrated-43db868.json` has SHA-256 `B6E8B860618E83DA0F851D4AC1A337ACA00F04CB47E16A950CFD5EB3FFF2AD64`, is byte-identical to `target/validation/task7-scheme-a-final-integrated-0e77537.json`, and keeps all 14 mandatory predicates true. It is an offline replay of the same old retained CSVs, not recapture.
- The prior `43db868` closure Release artifact is 42,283,520 bytes with SHA-256 `AFDB76ABA09468C9410B5FAC7E4FB5A46B67D0D928B6275512B207BAC3B2AE69`; that is offline build evidence only, not the current final build. Never substitute it for either sampled hash above, and never use the post-measurement correctness fixes or comparator replay as new live-interoperability evidence.

### Scheme A final offline closure (2026-08-31)

- The sole Mac live measurements remain serial `44a62ad` / `8CED2D0DB0788D34152AE498461A18F0255B103B3C20F87FCD2026932DD4C421` and candidate `c57dc77` / `4D1AECB691463E813F3C36122C9BC83464BB697028113C7AFE5814A0F102207F`. Everything below is offline and cannot upgrade the live result.
- `4a080cf` poisons the target runtime before returning `InvalidGeneration` when a cross-runtime, stale, or otherwise invalid opaque admission token is consumed, and that rejected consume adds no event, `Reset`, or wake. Bad input to `admit_generation` itself remains recoverable and does not poison. `8a32038` permits matching `display_count=2` to confirm Apple High Performance and activate the publisher, but only `display_count=1` is dynamic-resolution eligible; two displays close dynamic only.
- `aa560a3` leaves zero project-owned hot-path `wgpu::Device`, fault-observer-handle, or `GpuContext` clones in the affected paths. The accepted upstream wgpu 30 cost remains: one internal `DispatchDevice` clone per `push_error_scope`, three pushes per FRD fault scope, hence nine across routine acquisition/frame/record plus three for `CandidateBatch`. Do not restate this as absolute zero GPU clones.
- Comparator commits `cd01e78`, `18498ed`, `dec2cd6`, `3bde039`, `3db3b53`, and `50354fa` enforce exact case-sensitive event taxonomy and Ordinal identity, freeze session/generation at the Restore boundary, keep event timestamps in half-open phases, require process S0..S30 at 1 s +/-100 ms and five-second spans at 5 s +/-200 ms, enforce event-specific required/allowed/empty shapes, reject populated nonapplicable fields, require canonical unsigned-64 decimal event numbers, reject exact duplicates, accept optional revision for an installed-surface `CandidateBatch`, constrain full-string ASCII `run_id` to 1..64 `[A-Za-z0-9_-]` characters, and fail closed on `StableFault` or any invalid gate.
- No retained CSV was resampled. Windows PowerShell 5.1 output is 15,888 bytes / `B6E8B860618E83DA0F851D4AC1A337ACA00F04CB47E16A950CFD5EB3FFF2AD64`; pwsh 7 output is 6,354 bytes / `7651B13855D77C2D87BC805ABE4FC3D80B9750ACB8C68DBCB635F337831C850A`. Their bytes differ only by JSON whitespace formatting; parsed semantics are equal and both retain 14/14 predicates.
- Follow-up `42194bc` preserves typed batch execution failure across a simultaneous scope-finish panic; `af57bd5` closes the Apple wrapper admission-poison bypass; `3e375c0` bounds sparse type-1 publication to deterministic local groups instead of one global bounding rectangle.
- `2520452` makes the capture runner Windows PowerShell 5.1/pwsh 7 compatible without changing decoded text. `f7339cd` validates exact process-row shape, canonical numbers, and adjacent CPU deltas, and hides comparator child PowerShell windows. Both retained replays remain semantically equal at 14/14.
- `183b99d` changes only the Windows product Release to PE GUI subsystem 2 and presents fatal output through the native Windows dialog; debug remains console-backed.
- The final offline closure executable is 42,292,736 bytes with SHA-256 `21D307C64C6C153F4592FD1B1DC0C20856868F6DE239EDDD8DE087B6B894F6FC` and PE subsystem 2. This does not replace either sampled Mac binary or upgrade live evidence; the `43db868` artifact above remains merely the prior offline closure build.
