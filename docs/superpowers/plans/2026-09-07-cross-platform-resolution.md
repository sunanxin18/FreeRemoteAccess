...
- [x] Run the final workspace verification ladder, macOS package staging, and the available Windows-target check. Focused package tests, `cargo fmt --all -- --check`, `git diff --check`, macOS package staging, and the Windows `frd-app` target check passed. The workspace test was executed but remains blocked by the pre-existing `frd-icon-assets` deterministic derivative mismatch; the root binary and full RDP Windows target remain outside this host's linker/SDK capabilities.
