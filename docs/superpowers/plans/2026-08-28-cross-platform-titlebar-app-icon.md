...
**Tech Stack:** Rust 2021, egui 0.36.1, winit 0.30.13, wgpu 30.0.1, Win32/DWM through `windows-sys` 0.59, a pinned Apache-2.0 Material Symbols Rounded subset for title-bar controls, built-in imagegen, `image` and `ico` for deterministic asset export, `winresource` for the Windows executable resource.
...
- Cargo uses its normal default build parallelism. Avoid launching overlapping Cargo commands or duplicate client processes.
...
- [x] **Step 1: Write failing literal-mapping tests**
...
- [x] **Step 4: Run focused and owning-crate tests GREEN**

Run: `cargo test -p frd-ui-model && cargo test -p frd-app`

- [x] **Step 5: Review boundary mutations**
...
- [x] **Step 1: Write failing behavior tests for semantic slots**
...
- [x] **Step 3: Install and render official Material Symbols Rounded glyphs**

Vendor a deterministic offline font subset containing only the approved
connection, speaker, clipboard, and session-action symbols. Instantiate the
official variable font at optical size 24, weight 400, fill 0, and grade 0;
register it as an isolated egui named family and render the official codepoints
inside fixed 32-point interaction slots. Preserve visible keyboard focus,
`on_hover_text` tooltips, and widget accessibility labels. Unsupported
capabilities remain visible and disabled. Do not retain hand-drawn fallback
paths.

- [x] **Step 4: Remove post-connect text/toolbars from `show_session_page`**
...
- [x] **Step 5: Run the crate tests GREEN**
...
- [x] **Step 1: Write failing pure layout and hit-test tests**
...
- [x] **Step 3: Implement common geometry and platform trait**
...
- [x] **Step 4: Implement Windows DWM/non-client integration**
...
- [x] **Step 5: Connect window actions**
...
- [x] **Step 6: Run Windows shell tests GREEN**
...
- [x] **Step 1: Extend the existing failing geometry tests**
...
- [x] **Step 3: Replace `remote-toolbar` with title-bar chrome**
...
- [x] **Step 4: Dispatch chrome actions and preserve remote keyboard focus**
...
- [x] **Step 5: Run affected tests GREEN**
...
- [x] **Step 1: Generate the transparent canonical foreground**
...
- [x] **Step 2: Write failing asset-export tests**
...
- [x] **Step 5: Generate and verify committed assets**
...
- [x] **Step 1: Write failing window-configuration and resource tests**
...
- [x] **Step 3: Add runtime and executable icon wiring**
...
### Task 7: Default-Concurrency Verification and Windows Acceptance
...
- [x] **Step 1: Run formatting and complete tests with Cargo default concurrency**
...
cargo clippy -p freeremotedesk-windows -p frd-shell-desktop -p frd-ui-egui -p frd-ui-model -p frd-icon-assets --all-targets --no-deps -- -D warnings
```

- [x] **Step 2: Inspect packaged icon resources**
...
- [x] **Step 3: Run offline title-bar acceptance**

Use `--test-texture` at the monitor's native scale and use literal layout/input
tests for 100, 150, and 200 percent scaling. Do not change the user's global
Windows display setting solely for acceptance. Verify native drag, edge resize,
Snap Layout hit semantics, minimize, maximize/restore, close, centered glyphs,
tooltips, keyboard focus, and that no second toolbar appears.

- [x] **Step 4: Run one bounded live Apple session**

Load credentials only from the approved environment provider. Verify one
complete frame, title-bar controls do not overlap the remote image, deterministic
input-route tests still exclude title-bar events while preserving remote
keyboard routing, and Disconnect returns cleanly to the connection form. Do not
synthesize remote input during automated acceptance and do not launch a second
client.

- [x] **Step 5: Record evidence and audit every spec requirement**
