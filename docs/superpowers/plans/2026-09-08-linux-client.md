...
- 完整包：`5cd73ea` i686 job `101971932014` 完成应用测试、release 构建、包校验、实际解码器加载及产物上传；ARM64/x86_64 同轮在 zlib 系统依赖名单处失败。a60eebd 的 ARM64/x86_64 修正后 jobs 101983147418/101983147612 已通过完整包及目标解码加载，同轮 i686 仍在运行，不提前勾选同提交三目标包门禁。
...
- 共享帧事务：已提取 transaction_state 模块，保留原有类型重导出及 wgpu 行为；Metal 渲染 38 项及文档测试通过，完整工作区 1706 项通过。Linux GL 执行器尚未接入。
