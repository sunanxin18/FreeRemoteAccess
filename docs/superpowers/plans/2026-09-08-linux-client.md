...
- [x] 接入 Linux Vulkan/GLES 条件后端，平台组合根选择 Linux `VULKAN | GL`，
  保留 Windows DX12 与 macOS ARM64 Metal 行为；三目标软件 GL renderer fixture 通过。
- [ ] 在真实 X11/Wayland 宿主完成物理输入、硬件 GPU 与窗口装饰验收；合成 smoke
  不得替代真实 RDP 控制证明。
